...
class InvoiceViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAdminUser]  # محدودیت دسترسی به ادمین‌ها

    @action(detail=False, methods=['get'])
    def unprinted(self, request):
        """لیست فاکتورهای پرداخت شده که پرینت نشده‌اند"""
        invoices = Invoice.objects.filter(payment_status="پرداخت موفق")
